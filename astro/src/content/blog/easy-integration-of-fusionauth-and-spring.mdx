...
This post is old and no longer works. If you'd like to use Spring and FusionAuth, read [the Spring Boot FusionAuth quickstart](/docs/get-started/quickstarts/web/quickstart-java-springboot-web).
</Aside>
...
2. [Create an Application](/docs/get-started/core-concepts/applications)
...
4. Setup a basic Spring application and include the FusionAuth Spring Security library in your Maven POM file. (see a full example [pom.xml](https://github.com/FusionAuth/fusionauth-example-spring-security/blob/main/pom.xml))
...
5. Connect our library to your application using a Spring Security configuration.
    ...
    2. Spring handles the `/login` request by initiating an OAuth2 redirect to the [`authorize` endpoint](/docs/lifecycle/authenticate-users/oauth/endpoints#authorize) on the FusionAuth instance.
...
1. [Create](/docs/get-started/core-concepts/applications) and then copy your FusionAuth API key and your application Id into `application.properties`. Also update the base URL to be the location of your FusionAuth instance.
