...
![The FusionAuth admin UI showing the advanced search for users.](/img/docs/get-started/download-and-install/search/elastic-search.png)
...
The FusionAuth environment variables are the same regardless of which engine you choose. Note that `SEARCH_TYPE` uses the value `elasticsearch` for both Elasticsearch and OpenSearch, as FusionAuth uses a single setting for both engines:
...
## Addendum: Docker Compose Service Configurations

The OpenSearch and Elasticsearch `faSearch` service configurations differ in their image, environment variables, and volume path. Use whichever matches your chosen engine:

```yaml
# OpenSearch
faSearch:
  image: opensearchproject/opensearch:3.6.0
  container_name: faSearch
  environment:
