...
Switching to Elasticsearch or OpenSearch as the FusionAuth search provider gives users more powerful features, like:
...
If you're running FusionAuth directly on your machine without Docker, you can change this environment variable directly.
...
It's an easy mistake to install Elasticsearch and forget to toggle your `SEARCH_TYPE` variable. You can verify which type of search your FusionAuth instance uses by browsing to <Breadcrumb>System -> About</Breadcrumb>.
...
These URLs must point to two Elasticsearch instance in the same cluster. FusionAuth will fail to start if you try to run completely separate Elasticsearch nodes both using FusionAuth.
